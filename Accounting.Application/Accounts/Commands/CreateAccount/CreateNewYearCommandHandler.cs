...
namespace Accounting.Application.Accounts.Commands.CreateAccount {
    public class CreateNewYearCommandHandler : IRequestHandler<CreateNewYearCommand, Unit> {
        ...
        public async Task<Unit> Handle (CreateNewYearCommand request, CancellationToken cancellationToken) {
            ...
            var lastYear = _database.Accounts.Max (b => b.Year);
            ...
            var accounts = await _database.Accounts.Where (y => y.Year == lastYear)
            ...
            await _database.Accounts.AddRangeAsync (accountsList);
